...
On systems using SysV Init (e.g., Devuan), the role performs the
following additional steps when `wireguard.start_at_boot` is `true`:

* Installs the init script `wg0.sysvinit.sh` (from the `files/`
  directory within the role) to `/etc/init.d/wg0` with executable
  permissions (`0755`).
* Enables and starts the `wg0` service via the `service` module, which
  will invoke the init script.

The init script supports the standard SysV actions: `start`, `stop`,
`restart`, and `status`.  Internally it uses `wg-quick up wg0` and
`wg-quick down wg0` to bring the interface up and down, and `wg show
wg0` to report status.  The script is configured to run in runlevels 2,
3, 4, and 5, and to stop in runlevels 0, 1, and 6 (via LSB `Default-Start`
and `Default-Stop` headers).

Note: If `wireguard.start_at_boot` is `false` or not defined, the init
script will **not** be installed and the service will **not** be enabled
